...
def draw_polygon(img_array:np.ndarray, polygon:np.ndarray, color_rgb=[0,255,0]):
    output = np.copy(img_array)
    alpha = 0.4
    point_set =  np.array(polygon, dtype=np.int32)
    cv2.fillPoly(output, pts=[point_set], color=color_rgb)
    output_image_array = cv2.addWeighted(output, alpha, img_array, 1 - alpha, 0)
...
def draw_segments(img_array, polygons, color_rgb):
    ...
    output_image = Image.fromarray(output_image)
    return output_image
def draw_foul_volume_lines(img: Image, lines, line_color, fig='lines'):
    drw = ImageDraw.Draw(img, 'RGB')
    for l in lines:
        for i in range(1,len(l)):
            if(fig=='lines'):
                drw.line([tuple(l[i-1]),tuple(l[i])], fill=line_color, width=3)
            if(fig=='points'):
...
def draw_bbox(img_array:np.ndarray, bbox, color, text_bbox=''):
    ...
    draw.text((bbox[0],bbox[1]-80), class_name, fill=(0,0,0), font=font)
